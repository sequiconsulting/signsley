#!/usr/bin/env python3
"""
Netlify Python Function: XAdES Verification
Using pyhanko 0.31 for XML signature verification
"""
...
try:
    from routers.verify_xades import XAdESValidator
...
def handler(event, context):
    """
    Netlify Function handler for XAdES verification
    ...
    try:
        ...
        file_name = body.get("fileName", "unknown.xml")
        ...
        # Decode base64 data
        try:
            xml_data = base64.b64decode(file_data)
        ...
        if not ('<?xml' in xml_content or '<' in xml_content):
            return {
                "statusCode": 400,
                "headers": headers,
                "body": json.dumps({
        ...
        try:
            result = loop.run_until_complete(
                validator.verify_xades_signature(xml_data, file_name)
            )
        finally:
            loop.close()
        ...
        try:
            error_response = create_error_response(
                ...
                file_name=body.get("fileName", "unknown.xml") if 'body' in locals() else "unknown.xml",
...
if __name__ == "__main__":
    ...
    test_event = {
        "httpMethod": "POST",
        "body": json.dumps({
            "fileData": "PD94bWw=",  # "<?xml" in base64
            "fileName": "test.xml"
        })
    }
